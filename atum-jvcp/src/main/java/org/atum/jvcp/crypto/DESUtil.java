...
public class DESUtil {
	...
	public static byte[] stringToBytes(String s) {
		...
		if (hex.length == 1 && "".equals(hex[0]))
			return new byte[0];
		byte[] buf = new byte[hex.length];
		for (int i = 0; i < hex.length; i++)
			buf[i] = (byte) (Integer.parseInt(hex[i], 16) & 0xFF);
		return buf;
	}
	...
	public static String intsToString(int[] ints, int offs, int len) {
		StringBuffer sb = new StringBuffer();
		String bt;
		if (len > ints.length)
			len = ints.length;
		for (int i = 0; i < len && (i + offs < ints.length); i++) {
			bt = Integer.toHexString(ints[offs + i] & 0xFF);
			if (bt.length() == 1)
				sb.append('0');
			sb.append(bt);
			sb.append(' ');
